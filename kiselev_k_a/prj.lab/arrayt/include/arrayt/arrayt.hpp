...
public:
	...
	ArrayT();
	explicit ArrayT(const ptrdiff_t& len);
	ArrayT(const ptrdiff_t& len, const T& num);
	explicit ArrayT(const ArrayT& rhs);
	~ArrayT();

	ptrdiff_t ssize() const noexcept;
	void resize(const ptrdiff_t& new_size);
	void insert(const T& value, ptrdiff_t i);
...
template<typename T>
ArrayT<T>::ArrayT(const ptrdiff_t& len) {
	if (len <= 0) {
...
template<typename T>
ArrayT<T>::ArrayT(const ptrdiff_t& len, const T& num) {
	if (len <= 0) {
		throw std::invalid_argument("Expected positive value");
	}
...
template<typename T>
void ArrayT<T>::insert(const T& value, ptrdiff_t i) {
	if (i < 0 || i > size_) {
		throw std::invalid_argument("Index out of range");
	}
	...
	for (ptrdiff_t j = size_-1; j > i; j -= 1) {
